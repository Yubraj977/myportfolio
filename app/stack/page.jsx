'use client';

import { motion } from 'framer-motion';



const stack = {
  Languages: [
    { name: 'JavaScript', logo: '🟨' },
    { name: 'TypeScript', logo: '🔷' },
    { name: 'Python', logo: '🐍' },
    { name: 'Java', logo: '☕' },
    { name: 'HTML', logo: '📄' },
    { name: 'CSS', logo: '🎨' },
  ],
  'Frameworks & Libraries': [
    { name: 'React', logo: '⚛️' },
    { name: 'Next.js', logo: '🧭' },
    { name: 'Tailwind CSS', logo: '💨' },
    { name: 'Express.js', logo: '🚂' },
    { name: 'Redux', logo: '🔁' },
    { name: 'Framer Motion', logo: '🎞️' },
  ],
  'Tools & Services': [
    { name: 'Supabase', logo: '🟢' },
    { name: 'Firebase', logo: '🔥' },
    { name: 'Cloudinary', logo: '☁️' },
    { name: 'Clerk', logo: '🧑‍⚖️' },
    { name: 'Git', logo: '🔧' },
    { name: 'Vercel', logo: '▲' },
    { name: 'Netlify', logo: '🌐' },
  ],
  'Dev Tools': [
    { name: 'VSCode', logo: '🧠' },
    { name: 'Postman', logo: '📮' },
    { name: 'Figma', logo: '🎨' },
    { name: 'Copilot', logo: '🤖' },
    { name: 'iTerm2', logo: '💻' },
    { name: 'Notion', logo: '📓' },
  ],
  'Currently Learning': [
    { name: 'Docker', logo: '🐳' },
    { name: 'GraphQL', logo: '🧬' },
    { name: 'Rust', logo: '🦀' },
    { name: 'WebSockets', logo: '🔌' },
  ],
};

export default function StackPage() {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.9, rotateY: 10 }}
      animate={{ opacity: 1, scale: 1, rotateY: 0 }}
      transition={{ duration: 0.8, ease: 'easeOut' }}
      className="px-6 py-10 max-w-6xl mx-auto text-black dark:text-white"
      style={{ perspective: 1000 }}
    >
      <h1 className="text-4xl font-extrabold mb-10 text-center">🧱 My Stack</h1>

      <div className="space-y-12">
        {Object.entries(stack).map(([category, items]) => (
          <motion.div
            key={category}
            initial={{ opacity: 0, y: 50, rotateX: 5 }}
            whileInView={{ opacity: 1, y: 0, rotateX: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
          >
            <h2 className="text-2xl font-semibold mb-4 border-b pb-2 border-gray-300 dark:border-gray-700">
              {category}
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {items.map(({ name, logo }) => (
                <motion.div
                  key={name}
                  whileHover={{ scale: 1.1 }}
                  className="group relative bg-white dark:bg-[#1c1c1c] border dark:border-white/10 rounded-xl p-4 shadow-md dark:shadow-lg flex items-center justify-center flex-col gap-2 transition-all duration-300"
                >
                  <div className="text-3xl group-hover:scale-125 transition-transform duration-300">
                    {logo}
                  </div>
                  <div className="text-sm font-semibold text-center">{name}</div>

                  <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-20 transition-all duration-300 bg-blue-400 dark:bg-blue-600 blur-xl z-[-1]" />
                </motion.div>
              ))}
            </div>
          </motion.div>
        ))}
      </div>
    </motion.div>
  );
}